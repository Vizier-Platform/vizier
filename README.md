...
- `vizier deploy`  
  This process can take 7-12 minutes on average. The current status of deployment will print in your terminal. You can also view this within your AWS console under the newly created Cloudfront Stack.
...
- `vizier deploy`  
   This will re-deploy your site based on your configurations, and adds a step to your CI/CD pipeline.
...
- `git push`  
   The workflow files will be created within your project's `.github` folder upon `vizier deploy`.  
...
- Create 3 of these secrets with the following names:  
   `AWS_ACCESS_KEY_ID`  
   ...
   `AWS_REGION`  
   -The region of AWS you intend to deploy the site in.  
...
- `vizier destroy`  
   You will be prompted to confirm this action, as it WILL DESTROY ALL ASSOCIATED RESOURCES WITH THE STACK, INCLUDING YOUR DATABASE.  
...
- `vizier remove`  
  This deletes the `.vizier` folder within your project directory, as well as the corresponding configuration files.  
